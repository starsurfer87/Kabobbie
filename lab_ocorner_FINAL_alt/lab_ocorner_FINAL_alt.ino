...
int errorF, errorR;
int countSameF, countSameR, countBadF, countBadR;
...
void loop() {
  ...
  if (filteredDistF != 0 && filteredDistF < 5) {
  ...
  //   setLeftMotor(0);
  //   delay(250);
  } else if (countBadF >= 5 || countBadR >= 5 || countSameF >= 5 || countSameR >= 5){
    Serial.println("\tCorrection T3");
    setRightMotor(-POWER);
    setLeftMotor(-POWER);
    delay(250);
    setRightMotor(0);
...
void measureDistanceF() {
  ...
  Serial.print("\tCount Front");
  Serial.print(countSameF);
}
...
void measureDistanceR() {
  ...
  if (currDistR == prevDistR && currDistR != 0) {
    countSameR++;
  } else {
    countSameR = 0;
  }
